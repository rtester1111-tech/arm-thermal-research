...
def setupThermalModel(system, options):
    ...
    # Create thermal nodes
    node_die = ThermalNode()     # Die junction node
    node_pkg = ThermalNode()     # Package surface node
    node_amb = ThermalNode()     # Ambient environment node
    ...
    # CRITICAL: Attach thermal model to system FIRST, before any add*()
    # calls. The add*() methods internally do self._parent.thermal_components
    # .append(...), so self._parent must be valid (i.e., tm must already be
    # a child of system).
    ...
    # 5. Shunt capacitor: package -> ambient (Cauer topology)
    tm.addCapacitor(C_pkg, node_pkg, node_amb)

    # 6. Fix ambient node temperature

#!/usr/bin/env python3
"""
thermal_governor.py  -  V3 Advanced RC Thermal Model, DVFS Governor, & Experimental Sweeps

Physics-based 2-node and 3-node Cauer RC thermal models for ARM Cortex-X4 SoC simulation,
...
Generates 9 high-fidelity research-grade charts and updated JSON logs for V3 verification.
"""
...
# ---- 3-Node Cauer RC Thermal Parameters (V3) ----
# Topology: P -> [Die] --R1_3N-- [Pkg] --R2_3N-- [Heatsink] --R3_3N-- T_amb
#                 |C1_3N          |C2_3N              |C3_3N
R1_3N = 3.0     # K/W  - Die-to-package (TIM: Thermal Interface Material)
C1_3N = 0.5     # J/K  - Die thermal capacitance (thin silicon die)
R2_3N = 5.0     # K/W  - Package-to-heatsink (BGA substrate)
C2_3N = 2.0     # J/K  - Package thermal capacitance
R3_3N = 7.0     # K/W  - Heatsink-to-ambient (natural convection + radiation)
C3_3N = 10.0    # J/K  - Heatsink thermal capacitance (aluminum/copper mass)
# Note: R1_3N + R2_3N + R3_3N = 15.0 K/W — maintains steady-state compatibility with V1/V2
# Time constants: tau1=1.5s (die flash), tau2=10s (pkg soak), tau3=70s (HS inertia)
...
def rc_step_3node(T_die, T_pkg, T_hs, power, dt):
    """
    Cauer 3-node thermal model: Die -> Package -> Heatsink -> Ambient.

    Forward Euler with sub-stepping for numerical stability.
    Stability criterion: h < 2 * R_min * C_min = 2 * 3.0 * 0.5 = 3.0 s
    With 50 sub-steps of dt=5s, h=0.1s << 3.0s. Unconditionally stable.

    KCL equations:
      C1 dT1/dt = P - (T1-T2)/R1
      C2 dT2/dt = (T1-T2)/R1 - (T2-T3)/R2
      C3 dT3/dt = (T2-T3)/R2 - (T3-Tamb)/R3
    """
    sub_steps = 50
    sub_dt = dt / sub_steps
    for _ in range(sub_steps):
        dT_die = (power - (T_die - T_pkg) / R1_3N) / C1_3N * sub_dt
        dT_pkg = ((T_die - T_pkg) / R1_3N - (T_pkg - T_hs) / R2_3N) / C2_3N * sub_dt
        dT_hs  = ((T_pkg - T_hs) / R2_3N - (T_hs - T_AMBIENT) / R3_3N) / C3_3N * sub_dt
        T_die += dT_die
        T_pkg += dT_pkg
        T_hs  += dT_hs
    return T_die, T_pkg, T_hs
...
# Simulator Loop
# ---------------------------------------------------------------------------
def simulate(mode: str, workload_pattern: str = 'sustained', enable_leakage: bool = True,
             use_2node: bool = True, use_3node: bool = False):
    """
    ...
    use_3node=True overrides use_2node and uses the 3-node Cauer RC model.
    """
    ...
    for step_idx in range(steps):
        ...
        # Heat transfer step
        if use_3node:
            temp_die, temp_pkg, temp_hs = rc_step_3node(temp_die, temp_pkg, temp_hs, power, DT)
            temp = temp_die
        elif use_2node:
            temp_die, temp_pkg = rc_step_2node(temp_die, temp_pkg, power, DT)
            temp = temp_die
            temp_hs = T_AMBIENT  # not modeled in 2-node
        else:
            # 1-node equivalent
...
def simulate_biglittle():
    ...
    steps = int(DURATION / DT)
    big_temp, big_pkg, big_hs = T_AMBIENT, T_AMBIENT, T_AMBIENT
    mid_temp, mid_pkg, mid_hs = T_AMBIENT, T_AMBIENT, T_AMBIENT
    big_freq = 3.30
    mid_freq = 2.00
    MID_IPC_FACTOR = 0.60
    ...
    R3_MID, C3_MID = 5.0, 6.0  # Mid-core heatsink (shared package, smaller thermal mass)
    ...
    for step_idx in range(steps):
        ...
        big_temp, big_pkg, big_hs = rc_step_3node(big_temp, big_pkg, big_hs, big_power, DT)
        
        # Mid core Cauer 3-node simulation
        sub_steps = 50
        sub_dt = DT / sub_steps
        for _ in range(sub_steps):
            d1 = (mid_power - (mid_temp - mid_pkg) / R1_MID) / C1_MID * sub_dt
            d2 = ((mid_temp - mid_pkg) / R1_MID - (mid_pkg - mid_hs) / R2_MID) / C2_MID * sub_dt
            d3 = ((mid_pkg - mid_hs) / R2_MID - (mid_hs - T_AMBIENT) / R3_MID) / C3_MID * sub_dt
            mid_temp += d1
            mid_pkg += d2
            mid_hs  += d3
...
def generate_all_plots(results: dict, sweep: list, out_dir: Path):
    ...
    # 9. Chart 9: 3-Node vs 2-Node Cauer RC Comparison
    fig, axes = plt.subplots(2, 1, figsize=(14, 10), sharex=True)
    fig.suptitle('Chart 9: 3-Node vs 2-Node Cauer RC Thermal Transients (SVE2 Aggressive)',
                 fontsize=14, fontweight='bold')
    
    r_3n = simulate('sve2_aggressive', use_3node=True)
    
    # Top: Temperature traces for both models
    ax = axes[0]
    ax.plot(r_2n['time'], r_2n['temp_die'], color='#7B1FA2', linewidth=2.5, label='2-Node Die')
    ax.plot(r_2n['time'], r_2n['temp_pkg'], color='#7B1FA2', linewidth=1.5, linestyle=':', alpha=0.7, label='2-Node Package')
    ax.plot(r_3n['time'], r_3n['temp_die'], color='#D32F2F', linewidth=2.5, linestyle='--', label='3-Node Die')
    ax.plot(r_3n['time'], r_3n['temp_pkg'], color='#E64A19', linewidth=1.5, linestyle=':', alpha=0.7, label='3-Node Package')
    ax.plot(r_3n['time'], r_3n['temp_hs'],  color='#1976D2', linewidth=2.0, linestyle='-.', label='3-Node Heatsink')
    ax.axhline(T_THROTTLE, color='#C62828', linestyle=':', alpha=0.5, label=f'HW Throttle ({T_THROTTLE}°C)')
    ax.axhline(T_PROACTIVE, color='#F57F17', linestyle=':', alpha=0.5, label=f'Proactive ({T_PROACTIVE}°C)')
    ax.set_ylabel('Temperature (°C)', fontweight='bold')
    ax.set_ylim(20, 105)
    ax.legend(loc='upper left', fontsize=9, ncol=2)
    ax.set_title(f"Temperature Profiles | 2-Node Die Max: {r_2n['max_temp']:.1f}°C | "
                 f"3-Node Die Max: {r_3n['max_temp']:.1f}°C", fontsize=10, loc='left')
    
    # Bottom: Delta between 3-node and 2-node die temperatures
    ax2 = axes[1]
    delta_die = [t3 - t2 for t3, t2 in zip(r_3n['temp_die'], r_2n['temp_die'])]
    ax2.fill_between(r_2n['time'], 0, delta_die,
                     where=[d >= 0 for d in delta_die], color='#FFCDD2', alpha=0.7, label='3-Node warmer')
    ax2.fill_between(r_2n['time'], 0, delta_die,
                     where=[d < 0 for d in delta_die], color='#BBDEFB', alpha=0.7, label='2-Node warmer')
    ax2.plot(r_2n['time'], delta_die, color='#333333', linewidth=1.5)
    ax2.axhline(0, color='gray', linestyle='--', alpha=0.5)
    ax2.set_ylabel('ΔT_die (3N − 2N) (°C)', fontweight='bold')
    ax2.set_xlabel('Simulation Time (s)', fontweight='bold')
    ax2.set_title('Die Temperature Difference: 3-Node minus 2-Node', fontsize=10, loc='left')
    ax2.legend(loc='upper right', fontsize=9)
    
    plt.tight_layout()
    plt.savefig(out_dir / 'chart_rc_3node_comparison.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print("V3 Plotting completed successfully. 9 plots saved.")

#!/bin/bash
# run_o3cpu.sh - Run gem5 simulation using high-fidelity ArmO3CPU out-of-order model
# Estimated time: 10-40 mins per kernel, ~30-120 mins total.
#
# Suitable for execution on the user's Linux server.
# Ensure that gem5 and aarch64 cross-compiled binaries are correctly pathed.
...
echo "=== Starting gem5 Out-of-Order CPU (ArmO3CPU) Simulation ==="
echo "Project Directory: $PROJECT_DIR"
echo "gem5 Binary:       $GEM5_OPT"
echo "gem5 Config:       $GEM5_CONFIG"
echo "OPPs and IPCs will represent real pipeline and cache architecture."
echo "============================================================"

# Create outputs directories if they do not exist
mkdir -p "$PROJECT_DIR/m5out_scalar_o3"
mkdir -p "$PROJECT_DIR/m5out_neon_o3"
mkdir -p "$PROJECT_DIR/m5out_sve2_o3"
mkdir -p "$PROJECT_DIR/logs"
mkdir -p "$PROJECT_DIR/results"

# 1. Run Scalar simulation
echo -e "\n[1/3] Running Scalar O3CPU..."
time "$GEM5_OPT" --outdir="$PROJECT_DIR/m5out_scalar_o3" "$GEM5_CONFIG" \
  --cmd="$PROJECT_DIR/src/scalar.aarch64" \
  $O3_PARAMS > "$PROJECT_DIR/logs/gem5_scalar_o3.log" 2>&1

# 2. Run NEON simulation
echo -e "\n[2/3] Running NEON 128-bit O3CPU..."
time "$GEM5_OPT" --outdir="$PROJECT_DIR/m5out_neon_o3" "$GEM5_CONFIG" \
  --cmd="$PROJECT_DIR/src/neon.aarch64" \
  $O3_PARAMS > "$PROJECT_DIR/logs/gem5_neon_o3.log" 2>&1

# 3. Run SVE2 simulation
echo -e "\n[3/3] Running SVE2 256-bit O3CPU..."
time "$GEM5_OPT" --outdir="$PROJECT_DIR/m5out_sve2_o3" "$GEM5_CONFIG" \
  --cmd="$PROJECT_DIR/src/sve2.aarch64" \
  $O3_PARAMS \
  --param 'system.cpu[:].isa[:].sve_vl_se = 2' \
  > "$PROJECT_DIR/logs/gem5_sve2_o3.log" 2>&1

echo -e "\n[4/4] Parsing ArmO3CPU stats.txt results..."
...
for mode in ['scalar', 'neon', 'sve2']:
    path = f"{PROJECT_DIR}/m5out_{mode}_o3/stats.txt"
    if not os.path.exists(path):
        # Local fallback if directory structured differently
        path = f"m5out_{mode}_o3/stats.txt"
...
out_json = f"{PROJECT_DIR}/results/gem5_o3cpu_results.json"
